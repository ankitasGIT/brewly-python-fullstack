...
Production-grade **FastAPI** backend for the **Coffee shop**: order management with PostgreSQL, JWT auth (RBAC), payment and notification integration, idempotency, and strict order status flow.
...
![Testcase Screenshot](images/coverage_percentage.png)
...
![Testcase Screenshot](images/test_coverage.png)
...
| POST   | `/api/v1/admin/managers`                                        | Manager  | Create a new manager (manager-only)              |
...
| PATCH  | `/api/v1/admin/products/{product_id}/variations/{variation_id}` | Manager  | Update variation                                 |
| DELETE | `/api/v1/admin/products/{product_id}/variations/{variation_id}` | Manager  | Delete variation                                 |
