...
async def test_update_variation_success(
    ...
    test_variation: ProductVariation,
        ...
        body = {
            "name": "Triple Shot",
            "price_change_cents": 200,
        }

        response = await client.patch(
            f"/api/v1/admin/products/{test_product.id}/variations/{test_variation.id}",
            json=body,
            headers={"Authorization": f"Bearer {manager_token}"},
        )
        ...
        assert response.status_code == 200
        data = response.json()
        assert data["name"] == "Triple Shot"
        assert data["price_change_cents"] == 200
        assert data["id"] == str(test_variation.id)


@respx.mock
async def test_delete_variation_success(
    client,
    manager_token: str,
    test_product: Product,
    test_variation: ProductVariation,
